...
setInterval(() => {
	...
	if (delta < 0 && jsonParse === true) {
		console.log('Delta for previous allocated memory value:', redColor, delta.toFixed(3), 'MB', resetColor);
	} else if (delta > 0 && jsonParse === true) {
		console.log('Delta for previous allocated memory value:', greenColor, delta.toFixed(3), 'MB', resetColor);
	...
	if (freeMemory < DEFAULT_LIMIT && jsonParse === true) {
		console.log(redColor, '!!! ATTENTION: Available memory is under the defined limit !!!', resetColor);
